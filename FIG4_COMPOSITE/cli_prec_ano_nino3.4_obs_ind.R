...
lx_occ_lim<-which(lx_occ>65 & lx_occ<100)
#lx_acc_lim<-lx_acc[c(291:360,1:30)]
ly_occ_lim<-which(ly_occ>5 & ly_occ<40)
###########
...
lx_aph_lim<-which(lx_aph>65 & lx_aph<100)
ly_aph_lim<-which(ly_aph>5 & ly_aph<40)
####
...
lx_nce_lim<-which(lx_nce>65 & lx_nce<100)
ly_nce_lim<-which(ly_nce>5 & ly_nce<40)
####
NCE1<-NCE[lx_nce_lim,ly_nce_lim,]
NCEM<-apply(NCE1,c(1,2),mean,na.rm=T)
############ LOAD THE REANALYSIS DATA ######
load("nino3.4_PC_obs_ncep_JJAS.RData")
elnino<-which(PC_obs>quantile(PC_obs,0.75))
lanina<-which(PC_obs<quantile(PC_obs,0.25))
PC_obs_elni_nce<-apply(NCE1[,,elnino],c(1,2),mean) 
PC_obs_lani_nce<-apply(NCE1[,,lanina],c(1,2),mean) 
#########
...
se<-seq(-60,60,120/12)
par(mar=c(3,6,3,6))
image(lx_occ[lx_occ_lim],ly_occ[ly_occ_lim],PC_obs_elni-OCCM,xlab=~degree~E,ylab=~degree~N,main='',col=colorss,breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
box(which = "plot", lty = "solid",lwd=1.4)
mtext(side=3, "GPCC", col="black", font=2, cex=1.2)
...
#se<-seq(-60,60,120/12)
par(mar=c(3,6,3,6))
image(lx_aph[lx_aph_lim],ly_aph[ly_aph_lim],PC_obs_elni_aph-APHM,xlab=~degree~E,ylab=~degree~N,main='',col=colorss,breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
text(x=92, y=36, "El-Nino",col="black", font=2, cex=1.4)
box(which = "plot", lty = "solid",lwd=1.4)
mtext(side=3, "APHRODITE", col="black", font=2, cex=1.2)
world.add<-map("world", xlim=range(lx_aph[lx_aph_lim]), ylim=range(ly_aph[ly_aph_lim]),plot=T ,add=TRUE)
map("world2",col="black",lwd=2,interior=T,add=T)
##########
#se<-seq(-60,60,120/12)
par(mar=c(3,6,5,6))
image.plot(lx_nce[lx_nce_lim],ly_nce[ly_nce_lim],PC_obs_elni_nce-NCEM,xlab=~degree~E,ylab=~degree~N,main='',col=colorss,breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
text(x=92, y=36, "El-Nino",col="black", font=2, cex=1.4)
mtext(side=3,"NCEP", col="black", font=2, cex=1.2)
box(which = "plot", lty = "solid",lwd=1.4)
