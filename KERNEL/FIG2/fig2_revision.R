...
A<-CMI(y[i,94999:99999],x[i,95000:100000],x[i,94999:99999],1.5)
B<-MI(y[i,94999:99999],x[i,95000:100000],1.5)
C<-MI(x[i,94999:99999],x[i,95000:100000],1.5)
D<-CMI(x[i,94999:99999],x[i,95000:100000],y[i,94999:99999],1.5)
return(c(A,B,C,D))}
################### 1000 time units ############
sd1<-foreach(i = 1:9,.combine='rbind')%dopar%{
A<-MISD(y[i,95000:100000],x[i,95000:100000],1.5,100)    ## I(y_n-1;x_n)
B<-MISD(x[i,95000:100000],x[i,95000:100000],1.5,100)   ## I(x_n-1; x_n)
return(c(A,B))}
#########################    500 time units      ###########
sd2<-foreach(i = 1:9,.combine='rbind')%dopar%{
A<-SDCMI(y[i,95000:100000],x[i,95000:100000],x[i,95000:100000],1.5,100)   ## I(y_n-1,x_n,/x_n-1)
D<-SDCMI(x[i,95000:100000],x[i,95000:100000],y[i,95000:100000],1.5,100)   ## I(x_n-1,x_n/y_n-1))
return(c(A,D))
}
...
############################################################
cp<-seq(0,1,0.1)
plot(cp[1:9],IXY[1:9],type="l",col="red",lwd=2.5,ylim=c(0,0.4),lty=1,pch=18,xlab="coupling coefficient",ylab="Information exchange (nats)",cex.lab=1.2,cex.axis=1.5,main="Kernel")
