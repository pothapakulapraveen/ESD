...
B<-MISD(z[95000:100000],x[i,95000:100000],100)   ## I(x_n-1; x_n)
return(c(A,B))}
#########################    500 time units      ###########
sd2<-foreach(i = 1:11,.combine='rbind')%dopar%{
A<-SDCMI(y[95000:100000],x[i,95000:100000],z[95000:100000],100)   ## I(y_n-1,x_n,/x_n-1)
D<-SDCMI(z[95000:100000],x[i,95000:100000],y[95000:100000],100)   ## I(x_n-1,x_n/y_n-1))
return(c(A,D))
}
...
############################################################
cp<-seq(0,1,0.1)
plot(cp[1:9],IXY[1:9],type="l",col="red",lwd=2.5,ylim=c(0,0.6),lty=1,pch=18,xlab="coupling coefficient",ylab="Information exchange (nats)",cex.lab=1.2,cex.axis=1.5,main="Linear")
