...
dir1<-"/work/scratch/pp28jofe/PAPER-2/MPIESM-LR/"
nc1<-nc_open(paste(dir1,"tos_Omon_MPI-ESM-LR_historical_r1i1p1_195101-200512_int.nc",sep=""))
...
EOF_mpi<-  (-EOF*10)
PC_mpi<-   (-PC)
save(EOF_mpi,PC_mpi,file="iod_PC_mpi_JJAS.RData")
####
...
######## Figure 3 #######
par(mar=c(5,4,5,4))
########### CAUTION IT IS THE THIRD EOF ############
image.plot(lx_acc[lx_acc_lim],ly_acc[ly_acc_lim],-EOF*10,xlab=~degree~E,ylab=~degree~N,main='MPI-ESM-LR (EOF1)',col=rev(colorss),breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
...
#dev.print(postscript,'iod_mpi_PC.eps',width=8,height=6)
#dev.off()
