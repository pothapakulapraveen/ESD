...
foreach(j=1:100,.combine='cbind')%dopar%{
B<-MISD(x[i,95000:100000],x[i,95000:100000])   ## I(x_n-1; x_n)
}
sd2<-apply(sd2,c(1),sd)
#########################    500 time units      ###########
sd3<-foreach(i = 1:9,.combine='rbind',.options.nws=opts)%:%
  foreach(j=1:100,.combine='cbind')%dopar%{
A<-SDCMI(y[95000:100000],x[i,95000:100000],x[i,95000:100000])   ## I(y_n-1,x_n,/x_n-1)
...
############################################################
cp<-seq(0,1,0.1)
plot(cp[1:9],IXY[1:9],type="l",col="red",lwd=2.5,ylim=c(0,1),lty=1,pch=18,xlab="coupling coefficient",ylab="Information exchange (nats)",cex.lab=1.2,cex.axis=1.5,main="Kraskov")
