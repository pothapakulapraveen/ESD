...
dir1<-"/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/OBS/"
#nc1<-nc_open(paste(dir1,"HadISST_sst_in.nc",sep=""))
#nc1<-nc_open(paste(dir1,"HadISST_sst_in_70_05.nc",sep=""))
...
lx_acc_lim<-which(lx_acc>100 & lx_acc<300)
...
EOF_obs<- (-EOF*10)
#########
save(PC_obs,EOF_obs,file="nino3.4_PC_obs.RData")
########
...
######## Figure 3 #######
par(mar=c(5,4,5,4))
image.plot(lx_acc[lx_acc_lim],ly_acc[ly_acc_lim],-EOF*10,xlab=~degree~E,ylab=~degree~N,main='NCEP Reanalysis EOF1',col=rev(colorss),breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
...
#dev.print(postscript,'obs_2_nino.eps',width=12,height=6)
#dev.off()

##########    plot SST anomalies wih respect to the PC ####

#PC<- -PC
#elni<-which(PC>quantile(PC,prob=0.75))    ##### with respect to th quantiles > 75%
#lani<-which(PC<quantile(PC,prob=0.25))    ##### with respect to the quantile < 25%
#cli<-apply(PCC[,,bjja],c(1,2),mean)
#el<-apply(PCC[,,elni],c(1,2),mean)
#la<-apply(PCC[,,lani],c(1,2),mean)
#image.plot(lx_acc[lx_acc_lim],ly_acc[ly_acc_lim],(el-cli),xlab=~degree~E,ylab=~degree~N,main='HadSST',col=rev(colorss),breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
#box(which = "plot", lty = "solid",lwd=1.4)
#world.add<-map("world", xlim=range(lx_acc[lx_acc_lim]), ylim=range(ly_acc[ly_acc_lim]),plot=T ,add=TRUE)
#map("world2",col="black",lwd=2,interior=T,add=T)
