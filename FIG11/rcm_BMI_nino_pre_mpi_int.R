...
#####
###########
dir1<-"/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/WAS_22_MPI-ESM/"
nc1<-nc_open(paste(dir1,"pr_WAS-22_MPI-M-MPI-ESM-LR_historical_r1i1p1_CLMcom-ETH-COSMO-crCLIM-v1-1_v1_mon_195101-200512_APH_1.nc",sep=""))
...
PCC2<-PCC1[,,bjja]
#PCC2<-PCC2[,,201:420]
#save(PCC2,file="mpi_preprocess.RData")
#load("mpi_preprocess.RData")
#load("gpcc_preprocess.RData")
#####  REMOVING NAN Values #####
load("/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/PLOTS/COMPOSITE/pc_nino_mpi.RData")
nino<-PC_mpi
load("/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/PLOTS/COMPOSITE/iod_PC_mpi_JJAS.RData")
iod<-PC_mpi
...
mtext(side=3,"MPI-ESM-LR", col="black", font=2, cex=1.0)
world.add<-map("world", xlim=range(lx_acc[lx_acc_lim]), ylim=range(ly_acc[ly_acc_lim]),plot=T ,add=TRUE)
map("world2",col="black",lwd=2,interior=T,add=T)
########
par(mar=c(3,4,5,8))
...
box(which = "plot", lty = "solid",lwd=1.4)
text(x=90, y=36, "NET SYNERGY",col="black", font=2, cex=1.0)
mtext(side=3,"MPI-ESM-LR", col="black", font=2, cex=1.2)
world.add<-map("world", xlim=range(lx_acc[lx_acc_lim]), ylim=range(ly_acc[ly_acc_lim]),plot=T ,add=TRUE)
map("world2",col="black",lwd=2,interior=T,add=T)
