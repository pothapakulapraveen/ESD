...
nc7<-nc_open(paste(dir7,"pr_Amon_MPI-ESM-LR_historical_r1i1p1_195101-200512_APH_1.nc",sep=""))
MPI<-ncvar_get(nc7,"pr")
#MPI1<-MPI[,,bjja]
lx_mpi<-ncvar_get(nc7,"longitude")
ly_mpi<-ncvar_get(nc7,"latitude")
tm_mpi<-ncvar_get(nc7,"time")
lx_mpi_lim<-which(lx_mpi>55 & lx_mpi<100)
...
mtext(side=3, "MPI-ESM-LR", col="black", font=2, cex=1.2)
...
box(which = "plot", lty = "solid",lwd=1.4)
mtext(side=3, "Nor-ESM-M", col="black", font=2, cex=1.2)
text(x=92, y=36, "iod +ve",col="black", font=2, cex=1.4)
...
diff91[which(diff91>  60)]<- 60
image.plot.erwan(lx_ec[lx_ec_lim],ly_ec[ly_ec_lim],B91[lx_ec_lim,ly_ec_lim]-B9[lx_ec_lim,ly_ec_lim],xlab=~degree~E,ylab=~degree~N,main='',col=colorss,breaks=se,cex.main=1.4,cex.axis=2,cex.lab=1.4,legend.args=list(text='', side=4, font=2, line=2.5, cex=1))
box(which = "plot", lty = "solid",lwd=1.4)
...
#source("rot_NOR_el.R")
####    Here comes the RCM  ####
#source("rot_MPI_iodp.R")
#source("rot_NOR_iodp.R")
#source("rot_EC_iodp.R")
#dev.print(postscript,'Fig7a_new_iodp.eps',width=12,height=8)
