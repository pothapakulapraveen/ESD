...
nc1<-nc_open(paste(dir1,"gpcc_V5_1901-2005.nc",sep=""))
ACC<-ncvar_get(nc1,"prcp")
...
#PCC2<-PCC1[,,bjja]
#PCC2<-PCC2[,,201:420]
#save(PCC2,file="gpcc_preprocess.RData")
load("gpcc_preprocess.RData")
#####  REMOVING NAN Values #####
load("/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/PLOTS/COMPOSITE/nino3.4_PC_obs.RData")
nino<-PC_obs
load("/gpfs/ahrenshsmfs/user/pothapak/PAPER-2/PLOTS/COMPOSITE/iod_PC_obs_JJAS.RData")
iod<-PC_obs
...
box(which = "plot", lty = "solid",lwd=1.4)
text(x=88, y=37, "I(PREC;IOD,ENSO)",col="black", font=2, cex=1.0)
mtext(side=3,"GPCC", col="black", font=2, cex=1.2)
world.add<-map("world", xlim=range(lx_acc[lx_acc_lim]), ylim=range(ly_acc[ly_acc_lim]),plot=T ,add=TRUE)
map("world2",col="black",lwd=2,interior=T,add=T)
########
